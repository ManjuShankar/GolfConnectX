...
import {getMyCourses, getMyPosts, userProfileDetails, getFriends, searchFriends,
  searchGroups, editCourseNotes, addNewScore, profilegroupList, getNewScore, eventDetails, deleteScores, deleteScoresImage} from '../actions/profileActions';
...
class MyProfile extends React.Component {
  constructor(props) {
        ...
        this.state={
          ...
          errScore : "", getScores: {}, img:"",
          selectedImage:"",
          selectedScoreId:null,
          slectedImagesList: [], imageId:'',
        };
  ...
  onSaveNotes(id){
let innerDescValue = document.getElementById("txtnotes").value.replace(new RegExp('\r?\n','g'), '<br />');
    let notes = $('#txtnotes').val();
       this.props.editCourseNotes(innerDescValue,this.props.selectedCourse.course.id, id,  this.props.activeUser.token).then(()=>{
           this.setState({Notes:this.props.myProfile.notes});

           this.getnewScores();
             this.props.getCourseObject(this.props.selectedCourse.course.id, this.props.activeUser.token).then((id)=>{
    ...
    componentWillMount(){
      ...
      paramId =  (_.size(urlPath)>0)?(_.toInteger(urlPath[1])):(0);
          ...
          this.props.getMyCourses(this.props.activeUser.token, this.props.myProfile.MyProfile.profile.id).then(()=>{
           ...
           if(_.toInteger(paramId)==4){
             this.props.eventDetails(this.props.activeUser.token, this.props.myProfile.MyProfile.profile.id).then(()=>{
...
modifyParam(index){
      ...
      this.props.getMyPosts(this.props.activeUser.token, this.state.profileDetails.profile.id).then(()=>{
       ...
       this.props.profilegroupList(this.props.activeUser.token, this.state.profileDetails.profile.id).then(()=>{
       ...
       this.props.getFriends(this.props.activeUser.token, this.state.profileDetails.profile.id).then(()=>{
       ...
       this.props.eventDetails(this.props.activeUser.token, this.state.profileDetails.profile.id).then(()=>{
...
getnewScores(){
   ...
   this.props.getNewScore(this.props.activeUser.token, this.props.selectedCourse.course.id).then(()=>{
  ...
  noAlpha(e) {
    if(e.target.name == "lastScore"){
     ...
     if ((!re.test(e.key)) || (e.target.value.length >= 3)){
         ...
         ///this.isValidRange(_.trim(e.target.value))
         e.preventDefault();
     }
   }
}
...
deleteScore(id){
   this.props.deleteScores(this.props.activeUser.token, this.props.selectedCourse.course.id, this.state.selectedScoreId).then(()=>{
    this.getnewScores();
...
deleteScoreImg(scoreId){
  ...
  this.props.deleteScoresImage(this.props.activeUser.token, this.props.selectedCourse.course.id, this.state.selectedScoreId, id).then(()=>{
    this.getnewScores();
...
gotoCourse(){
  this.context.router.push("/courses_"+ this.state.courseDetails.course.id);
  location.reload();
}
onLikeClick(id){
        ...
        <div className="myProfile_Posts">
           <div className="img-cntnt col-sm-12 zeroPad">
            ...
            <div className="profileContent">
               {((_.size(this.state.profileDetails)>0) && isExistObj(this.state.profileDetails.profile))?(<div className="detailsPart col-sm-12 zeroPad">
         ...
         <img src={'http://'+ this.state.profileDetails.profile.profile_image_url} className="nameImg" />
                        ...
                        {isExistObj(this.state.profileDetails.profile) && (<div className="col-sm-3 pdng">
                         ...
                         {isExistObj(this.state.profileDetails.skills) && (<div className="margtp pdng col-sm-3">
                          ...
                          <div className="margtp pdng col-sm-3">
                             <div className="col-sm-12 pdng">
                                 ...
                                 {isExistObj(this.state.profileDetails.profile) &&  <div className="pdng fntlyt col-sm-6">{this.state.profileDetails.profile.is_private?'Private':'Public'}</div>}
                             </div>
                             <div className="col-sm-12 pdng">
                                 <div className="pdng fntbld col-sm-6">Handicap</div>
                                 {isExistObj(this.state.profileDetails.skills) && <div className="pdng fntlyt col-sm-6">{(this.state.profileDetails.skills.handicap==0)?0:this.state.profileDetails.skills.handicap}</div>}
                             </div>
                          </div>
                      </div>
         ...
         <TabPanel>
            {(this.state.ajaxCallInProgress)?(<div className="col-sm-12 bgwhite"><Spinner /></div>):((_.size(this.state.profileCoursesList)>0)?(<div className="images visitedDetails col-sm-12">
                    <div className="leftsideimgs col-sm-4 profilescroll">
                    {this.state.profileCoursesList.map((item, i) => {
                    return (<ul key={i}>
                    <li className={isExistObj(this.props.selectedCourse) && isExistObj(this.props.selectedCourse.course) && this.props.selectedCourse.course.id==item.id?"course_selected":"leftimg1"} onClick={this.showCourseDetails.bind(this,item.id)}>
                    <div className="nameoverimg">
                       {item.is_premium?<img src={"http://"+item.cover_image} className="img1"></img>:<img src="/assets/img/nonPremBanner.jpg" className="img1"></img>}
                        ...
                        <p className="ryt col-sm-6">{item.following_since}</p></li>
                    </ul>)
                    })}
                    </div>
                    ...
                    {(isExistObj(this.state.courseDetails) && isExistObj(this.state.courseDetails.course_user_details) && isExistObj(this.props.myProfile) && _.size(this.props.myProfile.MyProfileCourseList)>0)?(
                      <div className="rightsideimgs col-sm-8">
                        <div className="rytsideimg1">
                            <div className="nameoverimg">
                        {isExistObj(this.state.courseDetails.course) && this.state.courseDetails.course.is_premium?<img src={"http://"+this.props.selectedCourse.course.cover_image} className="img4"></img>:<img src="/assets/img/nonPremBanner.jpg" className="img4"></img>}
                             ...
                             {(this.state.courseDetails.course_user_details.is_following)?(<div className="following">
                              <span className="OrangeDot">
                                    <img src="/assets/img/icons/eclipse.png"/></span>Following</div>):(<div></div>)}
                                      ...
                                      {isExistObj(this.state.courseDetails.course) && <div className="cursor-pointer"><center>       <span className="center" onClick={this.gotoCourse.bind(this)}>{this.state.courseDetails.course.name}</span></center></div>}
      ...
      <div className="modal-content brdrRad0px">
        ...
        <div className="modal-body">
          <div className="form-group">
            ...
            <label>Date:</label>
           <DatePicker id="txtDate" className="form-control" ref="scoreDate" name="scoreDate" selected={moment(this.state.upComingeventDetail.end_date_format)} onChange={this.handleChange.bind(this, 'end_date')} maxDate={moment()} placeholderText="mm/dd/yyyy" />
          </div>
        </div>
        <div className="modal-footer">
          <button type="button" id ="saveScore" className="scoreSave-butn" onClick={this.addNewScores.bind(this)}>Save</button>
        </div>
      </div>
    </div>
  </div>
          ...
          {isExistObj(this.props.myProfile) && isExistObj(this.props.myProfile.getScores) ? this.props.myProfile.getScores.map((item,index)=>{
               return(<div className="col-sm-12 mb2pc" key={index}>
              ...
              {isExistObj(this.props.activeUser) && item.user.id==this.props.activeUser.id?<span className="glyphicon glyphicon-trash trashBlack" onClick={this.openDeleteScoreDiaglog.bind(this, item.id)} data-target="#ScoredeleteModal" data-toggle="modal" data-dropback="false"></span>:''}
              <div className="modal fade" id="ScoredeleteModal" role="dialog">
                ...
                <p>Notes<span className="glyphicon glyphicon-pencil" onClick={this.notesModal.bind(this, item.id)}></span></p>
                <p dangerouslySetInnerHTML={{__html: item.notes}} className="xx" maxLength="250"></p>
              </div>
              <div>
                <div className="col-sm-12 zeroPad">
                        ...
                        <div className="modal-content">
                          ...
                          <div className="modal-body">
                            <form id="imageUploadForm" name="imageUploadForm" ref="imageUploadForm" encType="multipart/form-data">
                              <input ref="file" id="file" type="file" name="file" className="uploadVisitImage" multiple  accept="image/*" />
                            </form>
                          </div>
                          <div className="modal-footer">
                            <button onClick={this.onImageUpload.bind(this, this.state.selectedScoreId)} type="button" className="upld_butns" data-dismiss="modal">Upload</button>
                          <button type="button" className="upld_butns" onClick={this.closeImageModal.bind(this)}>Close</button>
                      </div>
                  </div>
              </div>
          </div>
                  ...
                  <div className="col-sm-12 txtRyt zeroPad cursor-pointer">
                    {(_.size(item.images)>0)?(<span className="curPntr" onClick={this.openViewAllDialog.bind(this, item.id)}>View All &gt;</span>):(<span></span>)}
              </div>
                </div>
                        ...
                        <div className="round_close">
                          ...
                          <div className="opened_Image txtcenter">
                            <span className="thumbnail m0px galleryImg">
                              ...
                              <div className="edit" ><span  className="cursor-pointer"  data-toggle="modal" data-target="#vImagedeleteModal" ><i className="glyphicon glyphicon-remove left-91px"  ></i></span></div>
                             <div className="modal fade" id="vImagedeleteModal" role="dialog">
    <div className="modal-dialog modal-sm">
      <div className="modal-content">
        <div className="modal-header">
        ...
        <div className="modal-footer">
          <button type="button" className="checkng" onClick={this.vcloseModal.bind(this)}>No</button>
          <button type="button" className="checkng" onClick={this.vdeleteScoreImg.bind(this,item.id)}>Yes</button>
        </div>
      </div>
    </div>
  </div>
              ...
              <img src="assets/img/Add Photo@3x.png"  className="inlnImgs" onClick={this.openUploadImageDialog.bind(this, item.id)} />
                  {(_.size(item.images)>0 && item.images.map((childItem, index)=>{
                    return(<img src={"http://" + childItem.thumbnail} alt="upload_one" className="inlnImgs" onClick={this.openDialog.bind(this, childItem.id, item.id)} />)
                              ...
                              <div className="modal-body">
                                <div className="round_close">
                                  ...
                                  <div className="opened_Image txtcenter">
                                    <span className="thumbnail m0px galleryImg">
                                      <img src={this.state.selectedImage} alt="selectedImage" className="sliderImges" />
                                       <div className="edit" ><span  className="cursor-pointer"  data-toggle="modal" data-target="#ImagedeleteModal"><i className="glyphicon glyphicon-remove left-91px"  ></i></span></div>

                                        <div className="modal fade" id="ImagedeleteModal" role="dialog">
                                            <div className="modal-dialog modal-sm">
                                              <div className="modal-content">
                                                <div className="modal-header">
                                                  <h4>Delete Selected Image</h4>
                                                ...
                                                <div className="modal-footer">
                                                  <button type="button" className="checkng" onClick={this.closeModal.bind(this)}>No</button>
                                                  <button type="button" className="checkng" onClick={this.deleteScoreImg.bind(this,item.id)}>Yes</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="modal fade mt20pc" id="notesModal" role="dialog">
    <div className="modal-dialog modal-sm">
      ...
      <div className="modal-content">
        ...
        <div className="modal-body">
         ...
         <textarea className="wd100pc hgt130px" maxLength="250" id="txtnotes" onChange={this.onFieldChange}  defaultValue={item.notes!==""||item.notes!=null ?(item.notes):("")}></textarea>
        </div>
        <div className="modal-footer">
          <button type="button" className="submitButton"  onClick={this.onSaveNotes.bind(this,this.state.selectedScoreId)}>Submit</button>
        </div>
      </div>
                ...
                </div>):(<div><label>No Courses.</label></div>))}
        </TabPanel>
    <TabPanel>
                ...
                <div className=" col-sm-12">
                    ...
                    <div className="post1 col-sm-12">
                        <div className="subpost col-sm-12">
                            ...
                            <h3 className="mt1pc">{this.props.activeUser.name}</h3>
                            {/*<h4>{item.created}</h4>*/}
                            {item.object_name!=null?item.object_type=="Group"?<b>Posted on {item.object_type} <Link to={"groupMembers_" + item.object_id}><span className="txtUnderline"> {item.object_name}  </span></Link> on {item.created} </b>:item.object_type=="Course"?<b>Posted on {item.object_type} <Link to={"forumCourse_" + item.object_id}><span className="txtUnderline"> {item.object_name}  </span></Link> on {item.created} </b>:<b>Posted on {item.object_type} <Link to={"/events_"+item.object_id}><span className="txtUnderline"> {item.object_name}  </span></Link> on {item.created}</b>:''}
                            <p>{item.created_since}</p>
                        </div>
                            </div>
                        <div className="subpost-cntnt col-sm-12">
                        {isExistObj(this.props.activeUser) && this.props.activeUser.id==item.author.id?<span className="glyphicon glyphicon-trash fr cursor-pointer mt1pc" data-toggle="modal" data-target="#modalFileDlt"></span>:''}
                        ...
                        {_.size(item.comments)>0  && item.comments.map((childItem, i) => {
                        return(<div key={i}><div className="cmnt col-sm-12 txtRyt">
                        ...
                        <div className="Rosie-rply col-sm-12">
                          <div className="col-sm-1">
                            ...
                            <div className="Rosie-name col-sm-11">
                                <div className="col-sm-12">
                          {isExistObj(this.props.activeUser) && this.props.activeUser.id==childItem.author.id?<span className="glyphicon glyphicon-trash fr cursor-pointer mt1pc" onClick={this.deleteComments.bind(this,item.id, childItem.id)}></span>:''}
                                  <h3 className="col-sm-2 m0px">{childItem.author.first_name} {childItem.author.last_name}</h3>
...
function matchDispatchToProps(dispatch){
    return bindActionCreators({eventDetails, getMyCourses, getMyPosts, getCourseObject, createEvents,
     getCurrentEvent, userProfileDetails, getFriends, searchFriends,  searchGroups, editEvents,
     editCourseNotes, addNewScore, profilegroupList, getNewScore, deletePost, deleteComment, 
     deleteScores, deleteScoresImage, likePost}, dispatch);
}
